#include <stdint.h>
#include "./SubGlobals.h"

#ifndef MBConfig_H
#define MBConfig_H

/*=== Derek, please config modbus here ===*/
constexpr uint16_t SLAVE_ID = 12;
constexpr uint16_t MB_BAUD_RATE = 9600;
constexpr uint16_t MB_SERIAL_CONFIG = SERIAL_8E1;  // e.g., SERIAL_8E1, SERIAL_8N1
constexpr uint8_t MB_DATA_SIZE = 102;
constexpr uint8_t MB_DATA_BATCH_SIZE = 19;
constexpr uint8_t MB_READ_EPOCH_MAX = (MB_DATA_SIZE + MB_DATA_BATCH_SIZE - 1) / MB_DATA_BATCH_SIZE;
uint8_t mbReadEpochIdx = 0;
uint32_t mbDataHr[MB_DATA_SIZE] = {};  // read first n HR data, where n = MB_DATA_SIZE

/*=== End of config ===*/

class MBConfig {
private:
  float IEEEfloat(uint32_t val) {
    union {
      uint32_t i;
      float f;
    } u;
    u.i = val;
    return u.f;
  }

public:
  MBConfig(void) {}

  void init() {
    if (!mbRtuClient.begin(MB_BAUD_RATE, MB_SERIAL_CONFIG)) {
      Serial.println(F("Failed to start Modbus RTU Client!"));
      while (1) {};
    }
  }

  void loop() {
    if (mbTimer.autoTimeout(1000)) {
      this->readModbusData();
      this->debug();
    }
  }

  void readModbusData(void) {
    if (mbReadEpochIdx >= MB_READ_EPOCH_MAX) {
      mbReadEpochIdx = 0;
      return;
    }

    uint8_t startIdx = mbReadEpochIdx * MB_DATA_BATCH_SIZE;  // e.g., 0, 20, 40, 60, ...
    uint8_t endIdx = startIdx + MB_DATA_BATCH_SIZE;          // e.g., 21, 41, 61, 81, ...
    endIdx = endIdx >= MB_DATA_SIZE ? MB_DATA_SIZE : endIdx;

    mbRtuClient.requestFrom(SLAVE_ID, HOLDING_REGISTERS, startIdx, MB_DATA_BATCH_SIZE);

    Serial.print("startIdx: ");
    Serial.println(startIdx);
    Serial.print("endIdx: ");
    Serial.println(endIdx);

    for (uint8_t i = startIdx; i < endIdx; i++) {
      mbDataHr[i] = (uint32_t)mbRtuClient.read();
    }

    mbReadEpochIdx++;
  }

  // void readModbusData() {
  //   mbRtuClient.requestFrom(SLAVE_ID, HOLDING_REGISTERS, 0, 6);
  //   for (uint8_t i = 0; i < 6; i++) {
  //     Serial.println(mbRtuClient.read());
  //   }
  // }

  void debug() {
    Serial.println(F("\r\n>>> Print Data\r\n"));

    for (byte i = 0; i < MB_DATA_SIZE; i++) {
      Serial.print(i);
      Serial.print(F(": "));
      Serial.println(mbDataHr[i]);
    }
  }

  ~MBConfig() {}
};

extern MBConfig mbConfig;

#endif
